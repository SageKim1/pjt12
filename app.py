...
# ---------- 오답 노트 ----------
    ...
    if not st.session_state.wrong_answers:
        ...
        # ✅ 오답 통계 시각화 (과목별 틀린 문제 수)
        subject_stats = pd.Series([item["subject"] for item in st.session_state.wrong_answers]).value_counts()
        st.subheader("과목별 오답 분포")
        st.bar_chart(subject_stats)

        # ✅ 오답 상세 표시 + 선택 필터
        subjects_in_wrong = list(subject_stats.index)
        selected_filter = st.selectbox("과목별 필터", ["전체"] + subjects_in_wrong)

        pdf_export_text = []  # PDF용 텍스트 저장용 리스트
        ...
        for i, wrong_item in enumerate(st.session_state.wrong_answers):
            ...
            with st.expander(f"[{wrong_item['subject']}] {wrong_item['question'][:50]}..."):
                ...
                pdf_export_text.append(f"[{wrong_item['subject']}] {wrong_item['question']}")
                for j, option in enumerate(wrong_item['options']):
                    ...
                    if j == wrong_item['correct_answer']:
                        ...
                        icon = "❌ (내 답)"
                    line = f"{chr(65+j)}. {option} {icon}"
                    st.write(line)
                    pdf_export_text.append(line)
                st.write(f"**해설**: {wrong_item['explanation']}")
                pdf_export_text.append(f"해설: {wrong_item['explanation']}\n")

        # ✅ PDF 다운로드 기능
        def export_wrong_to_pdf():
            pdf = FPDF()
            pdf.add_page()
            pdf.set_font("Arial", size=12)
            for line in pdf_export_text:
                pdf.multi_cell(0, 10, line)
            file_path = "wrong_answers.pdf"
            pdf.output(file_path)
            return file_path

        if st.button("📥 오답노트 PDF 다운로드"):
            file_path = export_wrong_to_pdf()
            with open(file_path, "rb") as f:
                st.download_button("📄 다운로드", f, file_name="오답노트.pdf")
